 ...
 */
export function isQueryReadOnly(query, type) {
    const q = query.trim().toLowerCase();
    if (type === 'mongodb') {
        return (q.includes('.find(') ||
            q.includes('.count(') ||
 ...
 */
export async function executeQuery(config, query, onConfirmWrite) {
 ...
 */
function parseCliOutput(stdout, type) {
