...
import { loadAppConfig, writeAppConfig, getGlobalConfigPath } from '../config/index.js';
...
async function main() {
    ...
    const pinnedSkills = new Set(); // skills always injected regardless of matcher
    const mutedSkills = new Set(); // skills always excluded regardless of matcher
    let debugMode = true; // flow log trace — on by default, /debug off to disable
    let lastCollapsedOutput = null; // last tool output that was folded
    let skillNames = []; // populated async on startup for tab completion
    ...
    if (activeSmartConfig.provider === 'anthropic' && !config.anthropicApiKey) {
        console.error('Error: ANTHROPIC_API_KEY is not set but is required by the active "smart" model provider.');
        console.error('Please set it using: $env:ANTHROPIC_API_KEY="your-key" in PowerShell or set ANTHROPIC_API_KEY="your-key" in Command Prompt');
        process.exit(1);
    }
    else if (activeSmartConfig.provider === 'openai-compat' &&
        !config.openaiApiKey &&
        !config.openaiEndpoint?.includes('localhost') &&
        !config.openaiEndpoint?.includes('127.0.0.1')) {
        console.error('Error: OPENAI_API_KEY is not set but is required by the active "smart" OpenAI provider.');
        console.error('Please set it using: $env:OPENAI_API_KEY="your-key" in PowerShell or set OPENAI_API_KEY="your-key" in Command Prompt');
        process.exit(1);
    }
     ...
     */
    const estimateCost = (modelId, inputTokens, outputTokens, cacheHit) => {
     ...
     */
    const onConfirmDangerousTool = (toolCall, reason) => {
        if (spinner.active()) {
            spinner.stop();
        }
        return new Promise((resolve) => {
    ...
    const promptUser = () => {
        rl.question('\n👤 You > ', async (input) => {
            ...
            if ((trimmedInput.toLowerCase() === 'e' || trimmedInput.toLowerCase() === 'expand') &&
            ...
            if (trimmedInput.toLowerCase() === '/plans') {
                try {
                    ...
                    if (list.active.length === 0)
                        ...
                        list.active.forEach((p) => console.log(`  - ${p}`));
                    console.log('\n📁 Archived Plans:');
                    if (list.archived.length === 0)
                        console.log('  (none)');
                    else
                        list.archived.forEach((p) => console.log(`  - ${p}`));
                }
                catch (err) {
            ...
            try {
                conversationHistory = await orchestrator.runTurn(conversationHistory, (progress) => {
