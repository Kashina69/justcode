...
async function runOnboarding(rl) {
    ...
    let providerName = '';
    let providerType = 'openai-compat';
    let endpoint = '';
    let apiKey = '';
    ...
    if (providerType === 'anthropic') {
        ...
        if (endpoint)
            updateData.anthropicEndpoint = endpoint;
    }
    else {
        updateData.openaiApiKey = apiKey;
        if (endpoint)
            updateData.openaiEndpoint = endpoint;
    }
...
async function main() {
    ...
    const getModelInfo = (modelId) => {
        ...
        if (lower.includes('sonnet'))
            return 'Claude 3.5 Sonnet (Smart & capable)';
        if (lower.includes('haiku'))
            return 'Claude 3.5 Haiku (Fast & cheap)';
        if (lower.includes('gpt-4o-mini'))
            return 'GPT-4o Mini (Fast & cheap)';
        if (lower.includes('gpt-4o'))
            return 'GPT-4o (Smart & capable)';
        if (lower.includes('deepseek-chat'))
            return 'DeepSeek Chat (Capable & inexpensive)';
        if (lower.includes('deepseek-coder'))
            return 'DeepSeek Coder (Code specialist)';
        if (lower.includes('gemini-1.5-flash'))
            return 'Gemini 1.5 Flash (Fast & cheap)';
        if (lower.includes('gemini-1.5-pro'))
            return 'Gemini 1.5 Pro (Smart & capable)';
        return 'General purpose model';
    };
    const fetchModelsForProvider = async (providerType, apiKey, endpoint) => {
        if (providerType === 'anthropic') {
            return [
    ...
    const resumeSessionById = async (targetId) => {
        try {
    ...
    const handleModelsMenu = async () => {
        ...
        const selectedAlias = aliasOptions[aliasIndex];
    ...
    const handleCommand = async (trimmedInput) => {
