...
describe('DbExecutor', () => {
    ...
    describe('executeQuery', () => {
        it('should prompt confirmation for write query and block if rejected', async () => {
            const config = { type: 'sqlite', connectionString: 'test.db', name: 'test' };
            const onConfirmWrite = vi.fn().mockResolvedValue(false);
            await expect(executeQuery(config, 'DROP TABLE users;', onConfirmWrite)).rejects.toThrow('Database write operation cancelled by user');
