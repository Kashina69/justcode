...
All of those issues are now completely fixed, and I've verified every authentication and security boundary condition. 

I created a comprehensive security audit log artifact:
**[auth_security_log.md](file:///home/ubuntu/.gemini/antigravity/brain/e1b8495a-2e84-41dd-a042-631f614823bd/auth_security_log.md)**

### Security Flow Review & Verified Edge Cases:

1. **Path-Bypass Protection (Verified):**
   * If a user tries to access `/dashboard` or any protected dashboard path with an active recovery session (i.e. has the `requires_password_update` cookie), the middleware catches it and forces them back to `/update-password`.
2. **Auth-Page bypass (Verified):**
   * If they attempt to navigate to `/login` or `/signup` to evade the reset requirement, the middleware intercepts them *before* any other route checks and redirects them to `/update-password`.
3. **Session Bleed Prevention (Verified):**
   * Added `cookies().delete("requires_password_update")` to `signOutAction()` and `signInAction()` to ensure that:
     * If User A logs out without resetting, the cookie is destroyed immediately.
     * If a user logs in successfully via password, any leftover reset cookie state from a prior user or tab is cleanly purged.
4. **Magic Link Recovery Recovery Callback (Verified):**
   * If a user clicks an email recovery magic link, the request routes to `app/auth/callback/route.ts`. I added logic so that if the destination `next` is `/update-password` (or `type=recovery`), the backend drops the secure, HttpOnly `requires_password_update` cookie *prior* to redirecting them, bringing them cleanly into the route-guard system.
5. **No Optimistic Cooldown Lockout (Verified):**
   * The 90s resend timer is now bound purely to the success callback of the server action (`state.success`). If Supabase throws a rate-limit error or any validation error, the UI displays the error toast, the button remains active, and the user is not locked out.
6. **Input & Match Constraints (Verified):**
   * The button is disabled unless both fields are filled, and client + server validation schema rejects mismatching passwords.

The Next.js build and TypeScript compiler check are completely green! 

We can now begin Task 4: Modularizing `settings-page-client.tsx`. Should I initialize the `agency/components/modules/settings/` directory and begin extracting the components?

### User Input

give me a commit message for the current chagnes check by using git status.
...
Here is the commit message for the final security hardening and edge case fixes:

```text
fix(auth): secure password update flow, path protection, and resend cooldowns

- Restructured route guard in middleware to check the `requires_password_update` cookie before checking default auth pages, preventing password recovery bypasses.
- Changed recovery magic-link email callback to drop the `requires_password_update` cookie when routing to `/update-password`.
- Refactored signup and recovery OTP resend actions to return status objects rather than redirecting, avoiding rate-limit lockout issues.
- Extracted and unified OTP resending into a shared `AuthResendButton` component.
- Triggered cooldown loaders locally only upon receiving a successful resend response from the server, backed by `localStorage` persistence.
- Added explicit cookie purging of `requires_password_update` on successful login and logout to prevent session bleed.
```