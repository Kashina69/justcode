...
import { DbConfig, DbSchema, DbTable, DbColumn } from './types.js';
import { executeQuery } from './executor.js';

/**
 * Introspects the database structure to build a DbSchema object.
 */
export async function introspectSchema(config: DbConfig): Promise<DbSchema> {
  ...
  if (config.type === 'sqlite') {
    ...
    for (const tableName of tableNames) {
      ...
      const columns: DbColumn[] = colRes.rows.map((r) => {
 ...
 */
export function renderSchemaAscii(schema: DbSchema): string {
  if (schema.tables.length === 0) return 'No tables/collections found.';
  ...
  for (const table of schema.tables) {
    ...
    for (const col of table.columns) {
      ...
      if (col.isForeignKey && col.references) suffix += ` (FK -> ${col.references.table}.${col.references.column})`;
 ...
 */
export function renderSchemaMermaid(schema: DbSchema): string {
  let out = 'erDiagram\n';
 ...
 */
export async function saveSchemaToFile(schema: DbSchema, outputPath: string): Promise<void> {
