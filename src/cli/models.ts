 ...
 */
export const getModelInfo = (modelId: string): string => {
 ...
 */
export const fetchModelsForProvider = async (providerType: string, apiKey: string, endpoint?: string): Promise<{ id: string }[]> => {
...
export const handleModelsMenu = async () => {
  ...
  const selectedAlias = ALIAS_OPTIONS[aliasIndex] as 'fast' | 'smart' | 'planner';
